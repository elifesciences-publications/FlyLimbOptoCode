function [S] = getSlicesSimple(newData, varList, winlen, trigger)
% This function cuts the activate triggered time series from the full
% dataset
...
if any(t)
    ...
    % Remove triggering points that are too close to the start of the trajectory
    k((k - w) < 1) = [];
    ...
    if any(k)
        ...
        if any(k)
            ...
            % Cut out the slices
            slices = arrayfun(@(x) X(x-w:x+w,:), k, 'UniformOutput', false);
        end
    end
end
